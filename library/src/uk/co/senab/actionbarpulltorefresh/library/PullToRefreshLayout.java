...
/**
 * The main component of the library. You wrap the views you wish to be
 * 'pullable' within this layout. This layout is setup by using the
 * {@link ActionBarPullToRefresh} setup-wizard return by
 * 
 * @link ActionBarPullToRefresh#from(android.app.Activity)}.
 */
public class PullToRefreshLayout extends StickyListHeadersListView {
	...
	/**
	 * Set a
	 * {@link uk.co.senab.actionbarpulltorefresh.library.listeners.HeaderViewListener}
	 * which is called when the visibility state of the Header View has changed.
	 * 
	 * @param listener
	 */
	...
	void addRefreshableView(View view) {
		if (mPullToRefreshAttacher != null) {
			mPullToRefreshAttacher.addRefreshableView(view,
					getViewDelegateFromLayoutParams(view));
		}
	}
	...
	ViewDelegate getViewDelegateFromLayoutParams(View view) {
		if (view != null && view.getLayoutParams() instanceof LayoutParams) {
			...
			if (!TextUtils.isEmpty(clazzName)) {
				...
				return InstanceCreationUtils.instantiateViewDelegate(
						getContext(), clazzName);
			}
		}
		return null;
	}

	protected PullToRefreshAttacher createPullToRefreshAttacher(
			Activity activity, Options options) {
		return new PullToRefreshAttacher(activity, options != null ? options
				: new Options());
	}
	...
	public void setAdapter(StickyListHeadersAdapter adapter) {
		mExpandableStickyListHeadersAdapter = new ExpandableStickyListHeadersAdapter(
				adapter);
	...
	public void expand(long headerId) {
		...
		List<View> itemViews = mExpandableStickyListHeadersAdapter
				.getItemViewsByHeaderId(headerId);
		if (itemViews == null) {
			return;
		}
